...
function Cards() {
  return (
    <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto'>
        {projects.map(({name,image,alt,description,languages,github},index)=>{
            return(
                <div key={index} className='border-2 border-[#1E90FF] rounded-xl overflow-hidden flex flex-col justify-between md:hover:scale-105 transition-all duration-300 cursor-pointer'>
                    ...
                    <div className='flex gap-4 p-2'>
                            ...
                            <a href="" aria-label= "view the live demo" className='px-2 py-2 bg-blue-700 text-white rounded-xl font-semibold hover:bg-blue-900 transition'>Live Demo</a>
                    </div>
                </div>
            )
        })}
